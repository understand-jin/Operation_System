...
st.set_page_config(page_title="Operation_System - Inventory Matching", layout="wide")
st.title("⚖️ Inventory Matching")
    ...
    if st.button("🚀 데이터 분석 시작", key="btn_run_ia"):
        if not ia_file:
            ...
            if raw_df is not None:
                st.success(f"✅ 데이터 로드 완료 (메모리에서 처리됨)")
                    ...
                    if not col_mat or not col_sap or not col_wms:
                        ...
                        def get_styled_df(df, diff_color, diff_bg):
                            return df.style.set_properties(**{
                                ...
                                'background-color': '#f0f0f0'  # 연한 회색 (Light Gray)
