...
def get_csv_path(year: str, month: str, day: str, period: str, filename: str) -> Path:
    """4단계 계층 구조로 CSV 파일 경로 생성"""
    return BASE_DATA_DIR / year / month / day / period / filename

def save_csv_to_disk(df: pd.DataFrame, year: str, month: str, day: str, period: str, filename: str) -> Path:
    """DataFrame을 4단계 계층 구조로 로컬 디스크에 저장"""
    p = get_csv_path(year, month, day, period, filename)
    p.parent.mkdir(parents=True, exist_ok=True)  # ✅ 폴더 자동 생성
    df.to_csv(p, index=False, encoding="utf-8-sig")
    return p

def load_csv_from_disk(year: str, month: str, day: str, period: str, filename: str) -> pd.DataFrame:
    """4단계 계층 구조에서 CSV 파일 로드"""
    p = get_csv_path(year, month, day, period, filename)
...
# 자동 CSV 생성 헬퍼 함수
# -----------------------------
def detect_required_files(file_dict: dict) -> dict:
    """업로드된 파일 중 필요한 파일들을 찾아 반환"""
    file_map = {}
    
    for filename in file_dict.keys():
        filename_lower = filename.lower()
        
        if '매출a군' in filename_lower or '매출 a군' in filename_lower:
            file_map['sales_a'] = filename
        elif '배치번호별' in filename_lower and '재고' in filename_lower:
            file_map['batch_stock'] = filename
        elif '품절현황' in filename_lower:
            file_map['shortage'] = filename
        elif '청구실적' in filename_lower:
            file_map['billing'] = filename
    
    return file_map

def normalize_mat_code(s: pd.Series) -> pd.Series:
    s = s.astype(str).str.strip()
    s = s.str.replace("\u00A0", "", regex=False)   # NBSP
    s = s.str.replace(r"\.0$", "", regex=True)     # 8300492.0 -> 8300492
    s = s.str.replace(r"\s+", "", regex=True)      # 공백 제거
    return s

def pick_cols(df: pd.DataFrame, required: list) -> pd.DataFrame:
    ...
    for req in required:
        ...
        found = next((c for c in cols if c.replace(" ", "") == req_norm), None)
        if found is None:
            raise ValueError(f"필수 컬럼 없음: '{req}' (현재 컬럼: {cols})")
        picked.append(found)
    out = df[picked].copy()
    out.columns = required
    return out

def generate_stock_batch_csv(batch_df: pd.DataFrame, shortage_df: pd.DataFrame) -> pd.DataFrame:
    # 1) 배치 재고 기본 컬럼
    stock_base = pick_cols(batch_df, ["자재", "자재내역", "배치", "배치만료일", "가용재고 금액"])
    stock_base["자재"] = normalize_mat_code(stock_base["자재"])

    # 2) 품절현황 컬럼
    shortage_data = pick_cols(
        shortage_df,
        ["자재", "주문가능량(이동포함)", "3개월 평균출하", "전전월출하", "전월출하", "당월출하"]
    )
    shortage_data["자재"] = normalize_mat_code(shortage_data["자재"])

    # 3) 병합
    return stock_base.merge(shortage_data, on="자재", how="left")



def generate_stock_csv(shortage_df: pd.DataFrame) -> pd.DataFrame:
    """
    Stock.csv 생성
    - 품절현황조회_Data.xlsx에서 필요한 컬럼만 추출
    """
    try:
        # 품절현황에서 필요한 컬럼 추출
        required_cols = ['자재', '자재명', '주문가능량(이동포함)', '3개월 평균출하', 
                        '전전월출하', '전월출하', '당월출하']
        
        shortage_cols = shortage_df.columns.tolist()
        matched_cols = []
        for req_col in required_cols:
        # 1️⃣ 정확히 같은 컬럼 먼저 찾기
            if req_col in shortage_cols:
                matched_cols.append(req_col)
                continue

            # 2️⃣ 그 다음에만 유연 매칭
            for actual_col in shortage_cols:
                if req_col.replace(' ', '') == actual_col.replace(' ', ''):
                    matched_cols.append(actual_col)
                    break
        
        if len(matched_cols) < len(required_cols):
            raise ValueError(f"품절현황 파일에 필요한 컬럼이 없습니다. 필요: {required_cols}, 발견: {matched_cols}")
        
        stock_df = shortage_df[matched_cols].copy()
        stock_df.columns = required_cols  # 표준 컬럼명으로 변경
        
        # 🔥 자재 컬럼을 문자열로 변환 (데이터 타입 통일)
        stock_df['자재'] = stock_df['자재'].astype(str).str.strip()
        
        return stock_df
        
    except Exception as e:
        raise Exception(f"Stock.csv 생성 실패: {str(e)}")



def generate_a_product_csv(sales_a_df: pd.DataFrame, stock_df: pd.DataFrame) -> pd.DataFrame:
    """
    a_product.csv 생성
    - 매출 A군 리스트와 Stock 데이터를 자재 코드로 병합
    """
    try:
        # 자재 컬럼 찾기
        sales_cols = sales_a_df.columns.tolist()
        material_col = None
        ...
        if not material_col:
            raise ValueError("매출 A군 파일에 '자재' 컬럼이 없습니다.")
        
        # 🔥 데이터 타입 통일 (문자열로 변환)
        sales_a_copy = sales_a_df.copy()
        stock_copy = stock_df.copy()
        
        sales_a_copy[material_col] = sales_a_copy[material_col].astype(str).str.strip()
        stock_copy['자재'] = stock_copy['자재'].astype(str).str.strip()
        
        # 매출 A군 데이터와 Stock 데이터를 자재 코드로 병합
        a_product_final = sales_a_copy.merge(
            stock_copy, 
        ...
        return a_product_final
        
    except Exception as e:
        raise Exception(f"a_product.csv 생성 실패: {str(e)}")
